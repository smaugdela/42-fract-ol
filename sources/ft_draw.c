...
/*   Updated: 2021/12/10 13:52:28 by smagdela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	color_mix(int color1, int color2)
{
	...
	color_mix = 0;
	byte = &color_mix;
	*byte = (char)(ft_mean(t1, t2) << 24) | 0;
	*++byte = (char)(ft_mean(t1 * (color1 >> 16), t2 * (color2 >> 16)));
	*++byte = (char)(ft_mean(t1 * (color1 >> 8), t2 * (color2 >> 8)));
	*++byte = (char)(ft_mean(t1 * color1, t2 * color2));
	return (color_mix);
}
...
int draw_pixel(t_image *image, int x, int y, int color)
{
	...
	color = color_mix(*(int *)pixel, mlx_get_color_value(image->display->mlx_ptr, color));
	i = image->bpp;
	while (i >= 8)
	{
		i -= 8;
